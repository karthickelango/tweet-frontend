...
const Account = () => {
  ...
  const user = allUser.filter(e => e._id === activeUser);
  ...
  const deImg = 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80'
  return (
    <>
      {
        isLoading ? <Spinner /> :
            ...
            <ul role="list" className="divide-y divide-gray-100 profile-page">
              {
                user.map((obj, index) => (
                  <>
                    <li className="flex justify-between gap-x-6 pt-5 pb-2" key={index}>
                      <div className="flex min-w-0 gap-x-4">
                        <div className='position-relative profile-img'>
                          <img className="h-12 w-12 flex-none rounded-full bg-gray-50" src={obj.avatar === null ? deImg : `${BASE_URL}/${obj.avatar}`} alt="" />
                          <UploadImage activeUser={activeUser} />
                        </div>
                        <div className="min-w-0 flex-auto">
                          <p className="text-sm font-semibold leading-6 text-gray-900">{obj?.username}</p>
